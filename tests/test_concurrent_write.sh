...
TOTAL_LINES=$(wc -l < "$TARGET_ABS" | tr -d '[:space:]')  # BSD wc left-pads; strip it
...
LIVE_LOCKS=$(bash "$LOCK_SH" list | wc -l | tr -d '[:space:]')  # BSD wc left-pads; strip it
...
GARBLED=$(awk 'length > 100' "$TARGET_ABS" | wc -l | tr -d '[:space:]')  # BSD wc left-pads; strip it
