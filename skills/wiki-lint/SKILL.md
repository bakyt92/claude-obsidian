...
- **cli** — `obsidian-cli read path="$NOTE"` for individual reads; `obsidian-cli backlinks path="$NOTE"` natively handles the backlink graph (avoids re-rolling it via Grep); `obsidian-cli orphans` / `unresolved` surface dead-ends directly; see [`skills/wiki-cli/SKILL.md`](../wiki-cli/SKILL.md)
