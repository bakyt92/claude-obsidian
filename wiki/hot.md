...
2026-06-28 (latest): **Ingested [[Designing Data-Intensive Applications]]** (Martin Kleppmann, O'Reilly 2017, "DDIA"; early-release PDF, 491pp). Downloaded from a digital-garden mirror; `.raw/pdfs/designing-data-intensive-applications.pdf`. Outline extracted via pypdf `r.outline`. **Opens a brand-new domain**: distributed systems / data engineering — the vault's first non-ML/non-DS technical cluster. Created 3 pages: [[Martin Kleppmann]] (entity), [[Distributed Systems]] (domain hub), [[Reliability Scalability Maintainability]] (DDIA Ch1 framing). Updated [[O'Reilly Media]] (now publishes 2 vault books: DSFS + DDIA). **Self-referential hook worth remembering**: DDIA Ch7-9 (transactions/isolation/consensus) is the formal theory behind the vault's OWN v1.7 multi-writer advisory-locking model — linked [[Distributed Systems]] ↔ [[DragonScale Memory]]. Addresses `c-000030`–`c-000033`. Cluster map now: data infrastructure (DDIA) ⟂ data science (DS, DSFS, DS4Beginners) ⟂ ML/DL/LLMs.

2026-06-28 (latest): **Ingested [[Data Science for Beginners]]** (Microsoft, GitHub curriculum, 20 lessons/10 weeks, MIT license). WebFetch on the repo README was enough — no PDF. Created 2 pages: [[Microsoft]] (entity) + [[Data Science]] (concept, domain hub). **Editorial framing**: this is the *library-first, beginner* counterpart to [[Data Science from Scratch]] — same field, opposite pedagogy (Pandas/Matplotlib/Azure ML vs build-from-scratch). Noted as a "pedagogy contrast, not contradiction" and wired into [[From-Scratch Pedagogy]] as the live counterpoint. The [[Microsoft]] entity carries aliases `Microsoft Research` + `Azure Cloud Advocates`, which **resolves the long-dangling `[[Microsoft Research]]` link from PRML** (consolidated-entity approach, noted on the page). Addresses `c-000027`–`c-000029`. Reminder from prior ingest: slug-named source files need an `aliases:` field = spaced title, else `[[Spaced Title]]` links don't resolve in Obsidian (added it for this source up front).

2026-06-28 (latest): **Ingested [[Deep Learning (Goodfellow, Bengio, Courville)]]** (MIT Press 2016, 801pp — the foundational graduate DL text). Downloaded from IndiaAI/AIKosh static host; `.raw/pdfs/deep-learning-goodfellow.pdf`. **No embedded outline** (pypdf `r.outline` empty) — read the ToC from the front-matter text pages instead. Created 7 pages: [[Ian Goodfellow]], [[Yoshua Bengio]], [[MIT Press]] (entities), [[Backpropagation]], [[Generative Adversarial Network]], [[Recurrent Neural Network]] (concepts). **Key historical framing**: the book PREDATES the transformer (2016 vs "Attention Is All You Need" 2017), so it teaches sequence modeling via RNN/LSTM — filed a `> [!gap]` on the source + RNN pages marking the before/after of the attention revolution, and wired [[Recurrent Neural Network]] ↔ [[Self-Attention]] as the bridge. Addresses `c-000020`–`c-000026`. The AI cluster is now deep and well-connected: foundations (Linear Algebra, Backprop) → classical ML (PRML, DSFS) → deep nets (Deep Learning: CNN/RNN/GAN) → transformers/LLMs (Transformer, Self-Attention, MLLM, RAG).

2026-06-28 (latest): **Ingested [[Modern Large Language Models]]** (Daniel R. Holt, 2025, 721pp — a from-scratch transformer/LLM build guide). Local PDF at `~/Downloads`; copied to `.raw/pdfs/modern-large-language-models.pdf` and extracted metadata + embedded outline via `pypdf` (`r.outline` gave the full 24-chapter/7-part TOC cleanly — no need to read body pages). Created 5 pages: [[Daniel R. Holt]], [[PyTorch]] (entities), [[Transformer]], [[Self-Attention]], [[Retrieval-Augmented Generation]] (concepts). **Key vault connection**: the book's RAG chapter (Ch 21) + embeddings/vector-DB chapter (Ch 18) mirror the vault's own `wiki-retrieve` / [[Query-Time Retrieval]] machinery (contextual prefix + BM25 + cosine rerank). Filed RAG with an explicit "not a contradiction" callout reconciling it with [[Wiki vs RAG]]. Addresses `c-000014`–`c-000019`. Author entity is a deliberate stub (`> [!gap]`) — no independent bio, did not fabricate. The vault's ML/AI cluster is now substantial: PRML, DSFS, MLLM, Linear Algebra, Four Fundamental Subspaces, Transformer, Self-Attention, RAG.

2026-06-28 (later): **Ingested [[Data Science from Scratch]]** (Joel Grus, O'Reilly 2015, 1st ed., 330pp). The GitHub blob URL pointed at a binary PDF — WebFetch can't read those, so I `curl`ed the raw PDF to `.raw/pdfs/data-science-from-scratch.pdf` and extracted text with `pypdf` (poppler/pdftotext are NOT installed; `python3 -c "import pypdf"` works — use it for PDFs). Created 4 pages: [[Joel Grus]], [[O'Reilly Media]] (entities), [[From-Scratch Pedagogy]] (concept), [[DSFS vs PRML]] (comparison, in `wiki/comparisons/`). This is the **code-first pole** of the ML axis vs. PRML's theory-first; the comparison page makes the "which book" choice explicit. Cross-linked to the new [[Linear Algebra]] page (DSFS Ch.4 builds vectors/matrices from scratch). Addresses `c-000009`–`c-000013`. The vault now has a small ML/data-science cluster: PRML + DSFS + Linear Algebra + Four Fundamental Subspaces + 18.06.

2026-06-28: **Ingested [[MIT 18.06 Linear Algebra]]** (Gil Strang's course home page, https://web.mit.edu/18.06/www/). Opens a new **math / linear-algebra domain** in the vault. Created 4 pages: [[Gilbert Strang]] (entity), [[MIT OpenCourseWare]] (entity), [[Linear Algebra]] (concept, factorization-centric LU/QR/SΛS⁻¹/SVD view), [[Four Fundamental Subspaces]] (concept, Strang's signature framing). Cross-linked to [[Pattern Recognition and Machine Learning]] — 18.06 supplies the linear-algebra prerequisite PRML assumes. Addresses `c-000004`–`c-000008`. Also backfilled the index, which had never recorded the earlier PRML ingest under Sources. Note: WebFetch hallucinated an "Anthropic" mention on the course page; discarded.
